...
def prepare_dataset(path, name):
    ...
    # 读ICEWS时数据有时间，读完先不用时间
    entities, relations = set(), set()
    for f in files:
        file_path = os.path.join(path, f)
        to_read = open(file_path, 'r')
        for line in to_read.readlines():
            lhs, rel, rhs, time = line.strip().split('\t')
            entities.add(lhs)
            entities.add(rhs)
            relations.add(rel)
        to_read.close()
...
if __name__ == "__main__":
    # datasets = ['FB15K', 'WN', 'WN18RR', 'FB237', 'YAGO3-10', 'icews14']
    datasets = ['icews14']
    for d in datasets:
